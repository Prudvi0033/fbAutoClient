...
import { Plus, RefreshCw, Briefcase, MapPin, Building, Clock, Users, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
export default function JobPosts() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'POSTING': return 'text-blue-600';
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {jobs.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">Start by creating your first job post</p>
              <Link
                href="/job-posts/create"
                    ...
                    {job.postingStatus && (
                      <div className="bg-gray-50 rounded-lg p-3 mb-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-medium text-gray-700">Posting Status</span>
                          <span className="text-xs text-gray-600">
                            {job.postingStatus.totalGroups} groups
                          </span>
                        </div>
                        <div className="flex gap-4 text-xs">
                          <div className={`flex items-center gap-1 ${getStatusColor('SUCCESS')}`}>
                            {getStatusIcon('SUCCESS')}
                            {job.postingStatus.posted} Posted
                          </div>
                          {job.postingStatus.posting > 0 && (
                            <div className={`flex items-center gap-1 ${getStatusColor('POSTING')}`}>
